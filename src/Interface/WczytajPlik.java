...
public class WczytajPlik {
	...
	static Text text;
	public static OntModel modelnowy = ModelFactory.createOntologyModel(OntModelSpec.OWL_DL_MEM);
	...
	public static void main(String[] args) {
		try {
			WczytajPlik window = new WczytajPlik();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		if(Praca.wybor == true)
		lblWczytajPlik.setText("Wczytaj plik zawieraj¹cy kapsu³ki (onotologiê wielokapsu³kow¹)");
		else
		lblWczytajPlik.setText("Wczytaj plik zawieraj¹cy warstwê fundacjonistyczn¹");	
		...
		btnPrzegldaj.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try{					
					...
					modelnowy.read(stream,null);
					StringWriter writer = new StringWriter();
				modelnowy.write(writer, "RDF/XML-ABBREV") ;
		...
		btnDalej.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (Praca.wybor == true)
				{
					String kod="", uri;
					int i=0;
					Iterator classIter = modelnowy.listClasses();
				...
				shlCdnspl.dispose();
				WyborKapsulki kapsulka = new WyborKapsulki();
				kapsulka.open();
			}
			else{
				...
				MyUrinowy = "http://www.MyUri.com/myOntology";
				shlCdnspl.dispose();
