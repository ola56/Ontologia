...
public class WKaWyswietlThing {
	...
	Tree tree;
	String skrot = WKaNazwaKapsulkiThing.skrot;
	static String cel="";
	static String plan="";
	...
	public static void main(String[] args) {
		try {
			WKaWyswietlThing window = new WKaWyswietlThing();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		lblPodajCelPlanu.setBounds(10, 262, 463, 15);
		...
		lblPodajPlanReprezentujcy.setBounds(10, 310, 376, 15);
		...
		btnDodaj.setBounds(385, 329, 75, 25);
		btnDodaj.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
		  		...
		  		Cel = WczytajPlik.modelnowy.createClass(WczytajPlik.MyUrinowy + "#CelPlanu" + cel );
		  		Plan = WczytajPlik.modelnowy.createClass(WczytajPlik.MyUrinowy  + "#Plan" + plan );

		  		//Cel.addLabel("CelPlanu" + cel + WKaNazwaKapsulkiThing.nazwa1, "pl");
		  		//Cel.addLabel("GoalOf" + cel + WKaNazwaKapsulkiThing.tlumaczenie, "en");
		  		//Plan.addLabel("Plan" + plan + WKaNazwaKapsulkiThing.nazwa1, "pl");
		  		//Plan.addLabel("PlanOf" + plan + WKaNazwaKapsulkiThing.tlumaczenie, "en");
				...
				ObjectProperty jestGlownymCelem = WczytajPlik.modelnowy.createObjectProperty(WczytajPlik.MyUrinowy + "#jestGlownymCelem");
			    ObjectProperty maJakoGlownyCel = WczytajPlik.modelnowy.createObjectProperty(WczytajPlik.MyUrinowy + "#maJakoCelGlowny");
			      
			    Cel.addSuperClass(WczytajPlik.modelnowy.createSomeValuesFromRestriction(null, jestGlownymCelem, Plan));
			    Plan.addSuperClass(WczytajPlik.modelnowy.createSomeValuesFromRestriction(null, maJakoGlownyCel, Cel));
				MessageBox dialog = new MessageBox(shlCdnspl);
			    ...
			    {
			       shlCdnspl.dispose();
			       WKaEtykietyCelThing Dod = new WKaEtykietyCelThing();
			       Dod.open();				        	
			    }
		...
		lblPoKlikniciudodaj.setBounds(10, 360, 463, 32);
		...
		btnDalej.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				shlCdnspl.dispose();
				WKaDalejThing dalej = new WKaDalejThing();
				dalej.open();
			}
		});
		...
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				MessageBox dialog = new MessageBox(shlCdnspl);
				dialog.setMessage("W tym oknie zostanie przeprowadzona specjalizacja elementu D-"+WKaNazwaKapsulkiThing.skrot+" czyli zostan¹ dodane do D-"+WKaNazwaKapsulkiThing.skrot+" pojêcia zwi¹zane z planem oraz celem sytuacji, której dotyczy kapsu³ka.\r\n"
						...
						+ "Po klikniêciu \"Dodaj\" podany Cel oraz Plan zostan¹ dodane do elementu D-"+WKaNazwaKapsulkiThing.skrot+" i widok ontologii sie zaktualizuje o podan¹ zmiane. \r\n"
	...
	Iterator classIter = WczytajPlik.modelnowy.listClasses();
		...
		while (classIter.hasNext()) 
		{
			...
			if (((uri != null  && uri.startsWith("cDnSPL-Sytuacji" + WKaNazwaKapsulkiThing.nazwa1)) ) 
  					|| (uri != null  && uri.startsWith("Element-cDnSPL-Sytuacji" + WKaNazwaKapsulkiThing.nazwa1))
  					|| (uri != null && ontClass.isHierarchyRoot()))
				{
