...
public class EdyWyswietl {
	...
	public static void main(String[] args) {
		try {
			EdyWyswietl window = new EdyWyswietl();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		String nazwaa=EdyWyborKapsulki.nazwa;
  		int dlu1 = nazwaa.length();
  		nazwa1=nazwaa.substring(15, dlu1);
		//nazwa1 = EdyWyborKapsulki.nazwa;
  		char znak;
		...
		lblPodajPlanReprezentujcy.setBounds(10, 310, 383, 15);
		...
		btnDodaj.setBounds(381, 331, 75, 25);
		btnDodaj.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
		  		
		  		nazwaa2=text.getText();
		  		int dlu1 = nazwaa2.length();
		  		cel=nazwaa2.substring(8, dlu1);
		  		nazwaa1=text_1.getText();
		  		int dlu2 = nazwaa1.length();
		  		plan=nazwaa1.substring(4, dlu2);
	  		
		  		Cel = EdyWczytajPlik.modelnowy2.createClass(EdyWczytajPlik.MyUrinowy + "#CelPlanu" + cel);
		  		Plan = EdyWczytajPlik.modelnowy2.createClass(EdyWczytajPlik.MyUrinowy  + "#Plan" + plan);

		  		//Cel.addLabel("CelPlanu" + cel + EdyWyborKapsulki.nazwa, "pl");
		  		//Cel.addLabel("Cel" + cel + EdyWyborKapsulki.nazwa, "en");
		  		//Plan.addLabel("Plan" + plan + EdyWyborKapsulki.nazwa, "pl");
		  		//Plan.addLabel("Plan" + plan + EdyWyborKapsulki.nazwa, "en");
				...
				ObjectProperty jestGlownymCelem = EdyWczytajPlik.modelnowy2.createObjectProperty(EdyWczytajPlik.MyUrinowy + "#jestGlownymCelem");
			    ObjectProperty maJakoGlownyCel = EdyWczytajPlik.modelnowy2.createObjectProperty(EdyWczytajPlik.MyUrinowy + "#maJakoCelGlowny");
			      
			    Cel.addSuperClass(EdyWczytajPlik.modelnowy2.createSomeValuesFromRestriction(null, jestGlownymCelem, Plan));
			    Plan.addSuperClass(EdyWczytajPlik.modelnowy2.createSomeValuesFromRestriction(null, maJakoGlownyCel, Cel));
				MessageBox dialog = new MessageBox(shlCdnspl);
			    ...
			    {
			       shlCdnspl.dispose();
			       EdyEtykietyCel Dod = new EdyEtykietyCel();
			       Dod.open();				        	
			    }
		...
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				MessageBox dialog = new MessageBox(shlCdnspl);
				dialog.setMessage("W tym oknie zostanie przeprowadzona specjalizacja elementu D-"+skrot+" czyli zostan¹ dodane do D-"+skrot+" pojêcia zwi¹zane z planem oraz celem sytuacji, której dotyczy kapsu³ka.\r\n"
						...
						+ "Po klikniêciu \"Dodaj\" podany Cel oraz Plan zostan¹ dodane do elementu D-"+skrot+" i widok ontologii sie zaktualizuje o podan¹ zmiane. \r\n"
	...
	Iterator classIter = EdyWczytajPlik.modelnowy2.listClasses();
		...
		while (classIter.hasNext()) 
		{
			...
			if ((uri != null  && uri.startsWith("cDnSPL-Sytuacji" + nazwa1)) || (uri != null && uri.startsWith("Element-cDnSPL-Sytuacji" + nazwa1)) || (uri != null && ontClass.isHierarchyRoot()))
				{
