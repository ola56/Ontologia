...
public class EdyRole {

	protected Shell shlTworzenieKapsuekCdnspl;
	static EdyRole window = new EdyRole();
	Display display;
	private Text text;
	...
	protected void createContents() {
	    ...
	    lblWidokOntologii.setBounds(10, 94, 153, 15);
		...
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				{
					byt = text.getText();
					ontKlass = EdyWczytajPlik.modelnowy2.createClass(EdyWczytajPlik.MyUrinowy + "#" + text.getText());
					Iterator classIter = EdyWczytajPlik.modelnowy2.listClasses();
		   		while(classIter.hasNext())
		   			{
		   			OntClass ontClass = (OntClass) classIter.next();
		   			if(ontClass.getLocalName() != null && ontClass.getLocalName().startsWith("C-"+EdyWyswietl.skrot))
		   			{
			    ...
			    if (response1 == SWT.YES)
			    {
			       ...
			       EdyEtykietyByty byt = new EdyEtykietyByty();
			       byt.open();				        	
			    }
				...
				MessageBox msgBox = new MessageBox(shlTworzenieKapsuekCdnspl, SWT.ICON_QUESTION
			        ...
			        {
			        	shlTworzenieKapsuekCdnspl.dispose();
			        	EdyDodanieRol rola = new EdyDodanieRol();
			        	rola.open();				        	
			        }  
			    }
				}
		...
		btnNewButton_1.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				dialog.setMessage("Tutaj mo¿esz dodaæ pojêcia odpowiadaj¹ce bytom wydarzaj¹cym. S¹ to: OpisStanu, OpisZdarzenia, OpisAktywnosci oraz Zadanie.\nZosta³y one podane po prawej stronie. "
						...
						+ "Ni¿ej zosta³ zamieszczony przycisk \"Usuñ\", który usunie pojêcie zaznaczone w widoku ontologii, pod pojêciem C-"+EdyWyswietl.skrot
						+ "\r\nKlikaj¹c przycisk \"Dalej\" przejdziesz do reifikacji \n(dodawanie pojêæ do E-"+EdyWyswietl.skrot+" i C-"+EdyWyswietl.skrot+")");
				dialog.open();
			}
		});
		...
		label.setBounds(268, 93, 215, 21);
		
		Button btnUsu = new Button(composite, SWT.NONE);
		btnUsu.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
			     OntClass ontClass2 = EdyWczytajPlik.modelnowy2.getOntClass(EdyWczytajPlik.MyUrinowy +"#"+ nazwa);
			     OntClass ontClass1 = EdyWczytajPlik.modelnowy2.getOntClass(EdyWczytajPlik.MyUrinowy + "#C-" + EdyWyswietl.skrot);
		...
		btnUsu.setBounds(370, 487, 102, 25);
		btnUsu.setText("Usu\u0144");
	...
	public void refresh(Tree tree){
		...
		while (classIter.hasNext()) 
		{
			...
			if ((uri != null && uri.startsWith("Element-cDnSPL-Sytuacji" + EdyWyswietl.nazwa1)) )
				{
