...
public class WyborKapsulki {
	...
	public static void main(String[] args) {
		try {
			WyborKapsulki window = new WyborKapsulki();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		lblWybierzKapsukeKtr.setText("Wybierz, kt\u00F3r\u0105 kapsu\u0142k\u0119 chcesz specjalizowa\u0107");
		final Tree tree = new Tree(composite, SWT.FLAT | SWT.BORDER | SWT.V_SCROLL
			        | SWT.H_SCROLL);
		tree.setBounds(10, 35, 463, 200);
		tree.setFont(SWTResourceManager.getFont("Exo 2 Light", 9, SWT.NORMAL));
		
		   Button btnDodajNowKapsuke_1 = new Button(composite, SWT.NONE);
		   btnDodajNowKapsuke_1.addSelectionListener(new SelectionAdapter() {
		   	@Override
		   	public void widgetSelected(SelectionEvent e) {
		   		shlCdnspl.dispose();
		   		WKaNazwaKapsulkiThing thing = new WKaNazwaKapsulkiThing();
		   		thing.open();
		   		   		
		   	}
		   });
		   btnDodajNowKapsuke_1.setFont(SWTResourceManager.getFont("Exo 2 Light", 9, SWT.NORMAL));
		   btnDodajNowKapsuke_1.setBounds(272, 249, 201, 25);
		   btnDodajNowKapsuke_1.setText("Dodaj now\u0105 kapsu\u0142ke pod Thing");
		   
		   Button btnDodajNowKapsuke = new Button(composite, SWT.NONE);
		   btnDodajNowKapsuke.setFont(SWTResourceManager.getFont("Exo 2 Light", 9, SWT.NORMAL));
		   btnDodajNowKapsuke.setBounds(272, 304, 201, 25);
		   btnDodajNowKapsuke.addSelectionListener(new SelectionAdapter() {
		   	@Override
		   	public void widgetSelected(SelectionEvent e) {
		   		shlCdnspl.dispose();
		   		WKaNazwaKapsulki nowa = new WKaNazwaKapsulki();
		   		nowa.open();	   		
		   	}	   		
		   	
		   });
		   btnDodajNowKapsuke.setText("Dodaj pod t¹ klas\u0119 now\u0105 kapsu³kê");
