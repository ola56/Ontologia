...
public class Role {
	...
	static Role window = new Role();
	Display display;
	private Text text;
	static String rola;
	static String nazwa, bytWyd, byt;
	private Text text_1;
	...
	protected void createContents() {
		...
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				{
					byt = text.getText();
					ontKlass = WczytajWarstwe.model.createClass(WczytajWarstwe.MyUri + "#" + text.getText());
					Iterator classIter = WczytajWarstwe.model.listClasses();
		   		while(classIter.hasNext())
		   			{
		   			OntClass ontClass = (OntClass) classIter.next();
		   			if(ontClass.getLocalName() != null && ontClass.getLocalName().startsWith("C-"+NazwaKapsulki.skrot))
		   			{
			    ...
			    if (response1 == SWT.YES)
			    {
			       ...
			       EtykietyByty byt = new EtykietyByty();
			       byt.open();				        	
			    }
			    else{
				...
				MessageBox msgBox = new MessageBox(shlTworzenieKapsuekCdnspl, SWT.ICON_QUESTION
			        ...
			        {
			        	shlTworzenieKapsuekCdnspl.dispose();
			        	DodanieRol rola = new DodanieRol();
			        	rola.open();				        	
			        }
			    }			  
				}
		...
		btnNewButton_1.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				dialog.setMessage("Tutaj mo¿esz dodaæ pojêcia odpowiadaj¹ce bytom wydarzaj¹cym. S¹ to: OpisStanu, OpisZdarzenia, OpisAktywnosci oraz Zadanie.\nZosta³y one podane po prawej stronie. "
						...
						+ "Ni¿ej zosta³ zamieszczony przycisk \"Usuñ\", który usunie pojêcie zaznaczone w widoku ontologii, pod pojêciem C-"+NazwaKapsulki.skrot
						+ "\r\nKlikaj¹c przycisk \"Dalej\" przejdziesz do reifikacji \n(dodawanie pojêæ do E-"+NazwaKapsulki.skrot+" i C-"+NazwaKapsulki.skrot+")");
				dialog.open();
			}
		});
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				shlTworzenieKapsuekCdnspl.dispose();
				Reifikacja reifikacja = new Reifikacja();
				reifikacja.open();
			}
		});
		...
		button.setBounds(581, 556, 94, 25);
		 ...
		 Label lblZaznaczWybranyByt = new Label(composite, SWT.NONE);
		 lblZaznaczWybranyByt.setText("Zaznacz wybrany byt wydarzaj\u0105cy");
		 lblZaznaczWybranyByt.setFont(SWTResourceManager.getFont("Exo 2 Light", 10, SWT.BOLD));
		 lblZaznaczWybranyByt.setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
		 lblZaznaczWybranyByt.setBounds(268, 87, 215, 21);
		 
		 Label lblWPoniszymPolu = new Label(composite, SWT.NONE);
		 lblWPoniszymPolu.setText("Je\u017Celi chcesz usun\u0105\u0107 dodan\u0105 przez siebie rol\u0119 lub byt wydarzaj\u0105cy si\u0119 (spod C-S),\r\nto zaznacz w widoku wybrane poj\u0119cie i naci\u015Bnij \"Usu\u0144\"");
		 lblWPoniszymPolu.setForeground(SWTResourceManager.getColor(SWT.COLOR_WIDGET_DARK_SHADOW));
		 lblWPoniszymPolu.setFont(SWTResourceManager.getFont("Exo 2 Light", 9, SWT.NORMAL));
		 lblWPoniszymPolu.setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
		 lblWPoniszymPolu.setBounds(10, 448, 462, 35);
		 ...
		 btnNewButton_2.addSelectionListener(new SelectionAdapter() {
		 	@Override
		 	public void widgetSelected(SelectionEvent e) {
		 		  OntClass ontClass2 = WczytajWarstwe.model.getOntClass(WczytajWarstwe.MyUri +"#"+ nazwa);
				     OntClass ontClass1 = WczytajWarstwe.model.getOntClass(WczytajWarstwe.MyUri + "#C-" + WKaNazwaKapsulkiThing.skrot);
	...
	public void refresh(Tree tree){
		...
		while (classIter.hasNext()) 
		{
			...
			if ( (uri != null && uri.startsWith("Element-cDnSPL")) )
				{
